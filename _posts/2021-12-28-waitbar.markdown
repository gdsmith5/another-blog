...
See the code snippet below (which you can get [here]("{{ site.baseurl }}{% link ff_waitbar.m %}"){:target="_blank"}) for an example of how to use the  `waitbar()` function to display progress with a lengthy calculation (the Taylor series expansion of the exponential function):
